...
class MydemandPage extends React.Component {
  ...
  doMydemandApplyFlowQuery = function(values) {
    ...
    ItrsFlowApi.getMyDemandListAllApplyFlow(values,
      (success) => {
        this.setState({
          pagination: {
    ...
    <Spin spinning={ this.props.requesting } >
      ...
      expandedRowRender={ 
        record => (
          <span>
          { 
            record['applyFlowList'].length === 0 ? 
            "该条招聘需求下无相关招聘流程!"
            :
            <div>
              <MydemandFlowList
                  dataSource={ record['applyFlowList'] }
...
class MydemandFlowList extends React.Component {
  ...
  turnOperate(text, record) {
    ...
    for (let i = 0; i < text.length; i++) {
      // 防止key不唯一报错
      if (text[i] === '通过' || text[i] === '不通过') {
        // Popconfirm气泡确认框
        operateRes.push(
          <Popconfirm title="确定给出该简历筛选结果?" onConfirm={ () => this.confirmResult(text, record, i) } onCancel={ this.props.cancelResult } okText="确定" cancelText="取消" key={ 2*i }>
            <a key={ 2*i }>{ text[i] }</a>
          </Popconfirm>);
      } else {
        operateRes.push(
